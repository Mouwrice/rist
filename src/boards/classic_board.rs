...
impl ClassicBoard {
    pub fn new(players: usize) -> ClassicBoard {
        // CONTINENTS
...
impl Board for ClassicBoard {
    fn claim_territory(&mut self, territory_index: usize, player: &Rc<PlayerStruct>) {
        self.board.claim_territory(territory_index, player);
    }
}

impl Display for ClassicBoard {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        for territory in &self.board.territories {
            let player = territory.player.borrow();
            if player.is_none() {
                a.push(format!("* {:05} *", 0).white());
                n.push(format!("* {:5} *", territory.abbr).white());
                s.push("*******".white());
            } else if let Some(player) = &*territory.player.borrow() {
                n.push(
                    player
                        .upgrade()
                        .unwrap()
                        .colorize(format!("* {:5} *", territory.abbr)),
                );
                a.push(
                    player
                        .upgrade()
                ...
                s.push(player.upgrade().unwrap().colorize(String::from("*******")));
            }
        }
