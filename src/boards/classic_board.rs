...
use crate::boards::{BoardEnum, BoardStruct};
use crate::continent::Continent;
use crate::player::colorize;
use crate::territory::Territory;

pub fn new(players: usize) -> BoardEnum {
    // CONTINENTS
...
pub fn fmt(board: &BoardStruct, f: &mut Formatter<'_>) -> std::fmt::Result {
    ...
    for territory in &board.territories {
        ...
        if player.is_none() {
            ...
            n.push(colorize(
                &player.upgrade().unwrap(),
                format!("* {:5} *", territory.abbr),
            ));
            a.push(colorize(
                &player.upgrade().unwrap(),
                format!("* {:05} *", territory.armies.borrow()),
            ));
            s.push(colorize(
                &player.upgrade().unwrap(),
                String::from("*******"),
